...
def plot_loss_comparison(experiment_logs: Dict[str, List[dict]], save_path: str):
    ...
    for idx, (exp_name, histories) in enumerate(experiment_logs.items()):
        ...
        axes[idx].set_title(
            exp_name.replace("_", " ").title(), fontsize=12, color=COLOR_SCHEME["text"]
        )
    ...
    plt.tight_layout()
    plt.savefig(
        save_path, dpi=300, bbox_inches="tight", facecolor=COLOR_SCHEME["background"]
    )
    plt.close()
...
def create_generation_gif(text: str, max_tokens: int, save_path: str, fps: int = 30):
    """
    ...
    for i in range(min(max_tokens, len(text))):
        ...
        draw.text(
            (50, 30),
            f"Generated Text (token {i + 1}/{max_tokens})",
        ...
        draw.rectangle(
            [50, height - 50, width * progress, 10],
            outline=COLOR_SCHEME["accent3"],
            width=2,
        )
...
def visualize_attention_heatmaps(
    attention_weights: np.ndarray, tokens: List[str], save_path: str, num_heads: int = 6
):
    """
    ...
    fig, axes = plt.subplots(
        rows, cols, figsize=(6 * cols, 4 * rows), facecolor=COLOR_SCHEME["background"]
    )
    fig.suptitle(
    ...
    for head_idx in range(min(num_heads, rows * cols)):
        ...
        im = axes[row, col].imshow(attn, cmap="viridis", aspect="auto", vmin=0, vmax=1)
        axes[row, col].set_title(
            f"Head {head_idx}", fontsize=10, color=COLOR_SCHEME["text"]
        )
        ...
        if len(tokens) <= 20:
            ...
            axes[row, col].set_xticklabels(
                tokens, rotation=90, fontsize=8, color=COLOR_SCHEME["text"]
            )
            axes[row, col].set_yticks(range(len(tokens)))
            axes[row, col].set_yticklabels(
                tokens, fontsize=8, color=COLOR_SCHEME["text"]
            )
...
def create_dashboard(experiment_logs: Dict[str, Dict], save_path: str):
    ...
    for exp_name, logs in experiment_logs.items():
        ...
        ax1.plot(
            range(len(train_loss)), train_loss, label=f"{exp_name} (train)", linewidth=2
        )
        ax1.plot(
            range(len(val_loss)),
            val_loss,
    ...
    for exp_name, logs in experiment_logs.items():
        ...
        ax2.plot(range(len(grad_norm)), grad_norm, label=f"{exp_name}", linewidth=2)
    ax2.set_title(
    ...
    ax3.set_title(
        "Sample Generations", color=COLOR_SCHEME["text"], fontsize=14, weight="bold"
    )
    ...
    fig.suptitle(
        "Transformer Training Dashboard",
        fontsize=18,
        color=COLOR_SCHEME["text"],
        weight="bold",
    )
...
def create_all_visualizations():
    ...
    pos_enc_logs = {
        k: v for k, v in experiment_logs.items() if "pos" in k or "encoding" in k
    }
    norm_logs = {k: v for k, v in experiment_logs.items() if "norm" in k}

    # Create visualizations
    if pos_enc_logs:
        print("Creating loss comparison...")
        plot_loss_comparison(
            pos_enc_logs, os.path.join(output_dir, "pos_enc_loss_comparison.png")
        )

    if norm_logs:
        print("Creating gradient norm comparison...")
        plot_gradient_norms(
            norm_logs, os.path.join(output_dir, "gradient_norm_comparison.png")
        )

    # Dashboard
    print("Creating dashboard...")
    create_dashboard(
        experiment_logs, os.path.join(output_dir, "training_dashboard.png")
    )
